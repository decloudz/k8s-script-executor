...
// No ProcessID is sent; it's generated by the tracking service.
type ProcessTrackingCreatePayload struct {
	Name  string `json:"name"` // Script Name
	Stage string `json:"stage"`
	Group string `json:"group"`
	Label string `json:"label,omitempty"`
	// Add TrackingID from Task Service if needed for correlation?
...
func notifyProcessTrackingCreate(config *Config, payload ProcessTrackingCreatePayload) (int64, error) {
	if config.ProcessTrackingURL == "" {
		log.Printf("[ProcessTracking CREATE] Skipping notification: PROCESS_TRACKING_SERVICE_URL not set.")
		return 0, nil // Return 0 ID and no error if skipping
	}

	payloadBytes, err := json.Marshal(payload)
	if err != nil {
		log.Printf("[ProcessTracking CREATE] Error marshaling payload for Name %s: %v", payload.Name, err)
	...
	if err != nil {
		log.Printf("[ProcessTracking CREATE] Error creating request for Name %s: %v", payload.Name, err)
		return 0, fmt.Errorf("failed to create request: %w", err)
	}
	req.Header.Set("Content-Type", "application/json")

	log.Printf("[ProcessTracking CREATE] Sending creation request for Name: %s", payload.Name)
	resp, err := httpClient.Do(req)
	if err != nil {
		log.Printf("[ProcessTracking CREATE] Error sending notification for Name %s: %v", payload.Name, err)
	...
	if readErr != nil {
		log.Printf("[ProcessTracking CREATE] Failed to read response body for Name %s after status %d: %v", payload.Name, resp.StatusCode, readErr)
		// Decide if error even if status was 2xx? Maybe return 0 ID and let update fail later?
	}

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		log.Printf("[ProcessTracking CREATE] Notification failed for Name %s: Status %d, Body: %s", payload.Name, resp.StatusCode, string(bodyBytes))
		...
		if err := json.Unmarshal(bodyBytes, &createResp); err != nil {
			log.Printf("[ProcessTracking CREATE] Failed to unmarshal success response body for Name %s: %v. Body: %s", payload.Name, err, string(bodyBytes))
			return 0, fmt.Errorf("failed to parse create response: %w", err)
		}
		if createResp.ProcessID == 0 {
			log.Printf("[ProcessTracking CREATE] Got success status but processId was 0 or missing in response for Name %s. Body: %s", payload.Name, string(bodyBytes))
			return 0, fmt.Errorf("processId missing or zero in create response")
		}
		log.Printf("[ProcessTracking CREATE] Notification successful for Name %s. Received ProcessID: %d", payload.Name, createResp.ProcessID)
		return createResp.ProcessID, nil // Return the numeric ID
	}
}

// notifyProcessTrackingUpdate sends the final status (COMPLETED/FAILED) update using the numeric processID
func notifyProcessTrackingUpdate(config *Config, numericProcessID int64, payload ProcessTrackingUpdatePayload) {
	...
	// Construct the specific update URL with the numeric ID
...
func executeScript(c *gin.Context) {
	...
	numericProcessID, createErr := notifyProcessTrackingCreate(config, ProcessTrackingCreatePayload{
		...
		// TrackingID: request.TrackingID, // Optionally pass Task Service TrackingID here
	})

	if createErr != nil {
		// Log the creation error but continue execution? Or fail here?
		// Depending on requirements, might want to return an error to Task Service now.
		log.Printf("ERROR: Failed to create initial process tracking record for script '%s': %v. Continuing execution attempt...", actualScriptName, createErr)
		// Optionally: c.JSON(http.StatusInternalServerError, ...); return
	}

	// --- Resume normal execution flow, passing numericProcessID to updates ---
